...
class Dictionary
	...
	def possible_moves
		...
		@dictionary.each do |current_word|
			...
			# letter is deleted from word_letters and the list of available 
			# tiles
			...
			# will be false. Otherwise, it's added to the array of possible 
			# words
	...
	# Turns makeable words into array of their letters, puts each array into
		...
		if bound - incr_coord >= @len
			...
			while @i < @len do
				...
				@total += @pt.to_i * piece[@i]
					# This multiplies the multiplier on the board by the value 
					# of the letter. i and c are then incremented by 1
				@i += 1
				@c += 1
			end
